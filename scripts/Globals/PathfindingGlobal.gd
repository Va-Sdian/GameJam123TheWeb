...
#@onready var game_map = (get_tree().get_nodes_in_group("TileMap")[0])
@onready var game_map = null
#get_tree().get_nodes_in_group("TileMap")

func _ready():
	if get_tree().get_nodes_in_group("TileMap") != []: #создаёт ошибку, если загружать отдельно сцену врага
		#print(get_tree().get_nodes_in_group("TileMap"))
		game_map = (get_tree().get_nodes_in_group("TileMap")[0])
		_init_grid()

func _on_marker_positions_updated() -> void:
	print("_on_marker_positions_updated()")
...
func _init_grid() -> void:
	if game_map != null:
		...
		astar_grid.size = game_map.get_used_rect().size #Жалуется на .size
...
func find_path() -> void: #Не запускается, и судя по всему не работает
	print("find_path")
	path.clear()
...
#func clear_path() -> void: #Не работает
#	path.clear()
