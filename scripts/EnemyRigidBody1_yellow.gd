...
@export var MAX_SPEED = 90 #110 Скорость игрока
@export var FRICTION = 700
@onready var outOfTiLeMapFriction = 50
...
func _physics_process(delta):
	...
	match state:
		...
		STUN:
			...
			direction_to_target = Vector2.ZERO #Было сначало base вместо target
...
func stunned(_damage):
	...
	state = STUN
	StunT.start()
	#health -= damage #Не получает дамага при стане, хотя это вроде всё равно не работает
	if health <= 0:
		state = DEAD
